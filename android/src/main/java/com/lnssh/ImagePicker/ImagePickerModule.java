...
import static com.lnssh.ImagePicker.Utils.*;
...
public class ImagePickerModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    public void launchCamera(final ReadableMap options, final Callback callback) {
		Log.d("imagepicker","launchCamera");
    ...
    public void launchImageLibrary(final ReadableMap options, final Callback callback) {
		Log.d("imagepicker","launchImageLibrary");
        final Activity currentActivity = getCurrentActivity();
        if (currentActivity == null) {
			Log.d("imagepicker","currentActivity null");
            callback.invoke(getErrorMap(errOthers, "Activity error"));
            return;
        }
    ...
    void onAssetsObtained(List<Uri> fileUris) {
        try {
        	Log.d("imagepicker","onAssetsObtained1");
            callback.invoke(getResponseMap(fileUris, options, reactContext));
        } catch (RuntimeException exception) {
			Log.d("imagepicker","onAssetsObtained2");
