    ...
    if ([url.scheme hasPrefix:@"hwanc"]) {
        ...
        if ([url.host isEqualToString:@"link_mobileno"]) {
            ...
            if ([licenseStore isValid] == NO) {
                ...
                NSDictionary *query = [url.query dictionaryFromQueryComponents];
                NSString *routeTo = [query objectForKey:@"routeTo"][0];
                NSString *linkurl = [query objectForKey:@"url"][0];
                NSString *wcuri = [query objectForKey:@"wcuri"][0];
                NSString *wckey = [query objectForKey:@"key"][0];
                DiscoverController *discoverController = [DiscoverController sharedInstance];
                [discoverController setRouteTo:routeTo];
                [discoverController setLinkUrl:linkurl];
                [discoverController setWcUrl:wcuri];
                [discoverController setWcKey:wckey];
                dispatch_async(dispatch_get_main_queue(), ^{
                        UIViewController *currentVC = [AppDelegate sharedAppDelegate].window.rootViewController;
                    [currentVC presentViewController:discoverController animated:YES completion:nil];
                });
                return YES;
            } else {
                if ([LicenseStore requiresLicenseKey] == true) {
...
+ (BOOL)handleShortCutItem:(UIApplicationShortcutItem *)shortCutItem {
    if ([shortCutItem.type isEqualToString:@"ch.hwanc.newmessage"]) {
        [self composeMessage];
        
        return YES;
    } else if ([shortCutItem.type isEqualToString:@"ch.hwanc.myid"]) {
        UITabBarController *mainTabBar = [AppDelegate getMainTabBarController];
        mainTabBar.selectedIndex = kMyIdentityTabBarIndex;

        return YES;
    } else if ([shortCutItem.type isEqualToString:@"ch.hwanc.scanid"]) {
